...
@endphp
    ...
    <div class="card">
        <a href="@if(is_null($course->external_link)) {{ route('courses.show', $course->slug) }}?auth_token={{ request()->get('auth_token') }} @else {{ $course->external_link }}@endif" target="_blank" >

        <div class="lesson_name">
            <div class="bg-primary d-flex justify-content-center align-items-center flex-column position-relative img-overlay">
                <img src="{{ optional($course->episodes->where('show_status',1)->first())->thumb_path }}" alt="course-img" class="img-fluid">
                ...
                <a href="#" class="btn bg-secondary btn-sm text-white top-0 position-absolute end-0 rounded-0" title="buy-button">
                    ...
                    @else
                        <i class="icofont-cart-alt me-2"></i>
                    @endif
                </a>
            </div>
        </div>
            ...
            <div class="row g-2">
                <div class="col-6">
                    <div class="d-flex align-items-center">
                        ...
                        <span class="ms-2">{{ $course->episodes->where('show_status',1)->count() ?? 0 }} Lessons</span>
                    </div>
                </div>
                <div class="col-6">
                    <div class="d-flex align-items-center">
